...
def canonicalize_label(raw_label: str, case_type: str) -> str:
    ...
    Args:
        raw_label: Uppercase label string as it appears in the input CSV
            (e.g. "DECISÃO MANTIDA", "PARCIAL", "FAVORÁVEL").
        case_type: Either "dv" or "boc", selecting which mapping in
            `LABEL_CANONICALIZE` to use.

    Returns:
        The canonical lowercase, underscore-separated label string used in
        LLM output JSON and prompt construction (e.g. "decisao_mantida").
    ...
    Raises:
        ValueError: If `raw_label` is not a recognized label for `case_type`.
    """
