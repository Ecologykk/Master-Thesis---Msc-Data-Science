...
class LegalDocumentOrchestrator:
    ...
    def extract_links_interactive(self, search_type, tribunal_url, tribunal_id):
        ...
        try:
            ...
            if search_type == "TR":
                ...
                print("=" * 80)
                print(
                    "1. Search by keywords (e.g., 'violência doméstica', 'incumprimento de contrato')"
                )
                print("2. You'll see a list of possible DESCRIPTORS")
                print(
                    "3. ⚡ CLICK on a specific descriptor (e.g., 'VÍTIMA DE VIOLÊNCIA DOMÉSTICA')"
                )
            elif search_type == "JP":
                print("⚠️  WORKFLOW FOR PEACE COURTS (Contract Breach):")
                print("=" * 80)
                print(
                    "1. Search by descriptors/fields (e.g., 'incumprimento de contrato')"
                )
            ...
            print(
                "6. If there are multiple result pages, navigate between them and repeat"
            )
            print("=" * 80 + "\n")
            ...
            while True:
                ...
                if choice == "1":
                            ...
                            if "ecli-item-anchor" in link.get("class", []):
                                if href.startswith("/ecli/"):
                                    if not href.startswith("http"):
                                        href = (
                                            "https://jurisprudencia.csm.org.pt" + href
                                        )
                                    page_links.add(href)
                        else:
                    ...
                    if len(page_links) == 0:
                        ...
                        print(
                            "   1. You're on the DESCRIPTOR LIST page (need to click a descriptor)"
                        )
                    ...
                    try:
                        ...
                        if pagination:
                            ...
                            total_results = soup.find(
                                "span", class_="dynatable-record-count"
                            )
                            if total_results:
                                total = int(total_results.text)
                                max_pages = (
                                    total + 9
                                ) // 10  # Assuming 10 results per page
                            else:
                                max_pages = 200  # Conservative fallback
                        print(f"   📊 Detected {max_pages} total pages")
                    except Exception as e:
                        print(
                            f"   ⚠️  Could not detect max pages: {e}. Using fallback of 100."
                        )
                        max_pages = 200
                    ...
                    while current_page <= max_pages:
                        ...
                        print(
                            f"   ✅ Extracted {len(page_links)} links from page {current_page}"
                        )
                        print(f"   📈 New unique links: {len(new_links)}")
                        print(f"   📊 Total accumulated: {len(all_links)} unique links")
                        ...
                        try:
                            ...
                            next_buttons = driver.find_elements(
                                "css selector", "a.dynatable-page-next"
                            )
                            next_button = None
                            ...
                            for btn in next_buttons:
                                if (
                                    "Próximo" in btn.text
                                    and "dynatable-disabled-page"
                                    not in btn.get_attribute("class")
                                ):
                                    next_button = btn
                                    break
                            ...
                            if next_button:
                                print(
                                    f"   🔄 Clicking 'Próximo' to go to page {current_page + 1}..."
                                )
                                ...
                                print(
                                    f"   ⚠️  No enabled 'Próximo' button found. Reached end at page {current_page}."
                                )
                                break
                            ...
                            try:
                                time.sleep(2)
                                driver.execute_script(
                                    "document.querySelector('a.dynatable-page-next:not(.dynatable-disabled-page)').click();"
                                )
                                time.sleep(4)
                                current_page += 1
        ...
        for i, url in enumerate(links, 1):
            ...
            try:
                ...
                if doc:
                    ...
                    print(
                        f"[{i}/{len(links)}] ✅ Success! Process: {doc.get('n_processo', 'N/A')}"
                    )
                else:
    ...
    async def run(self, preset_choice=None, preset_court=None, preset_limit=None):
        ...
        if choice == "1":
            ...
            tribunal_id = tribunal_id_map.get(
                court_code, f"jtr{court_code[-1].lower()}"
            )
            ...
            documents, failed = await self.scrape_documents(
                links_to_scrape, self.dv_scraper, court_code, "VIOLÊNCIA DOMÉSTICA"
            )
            ...
            # Scrape documents
            documents, failed = await self.scrape_documents(
                links_to_scrape, self.cb_scraper, "JP", "INCUMPRIMENTO DE CONTRATOS"
            )
            ...
            print(
                "💡 Note: May contain duplicates with DGSI - check URLs in post-processing"
            )
            ...
            documents, failed = await self.scrape_documents(
                links_to_scrape, self.cb_scraper, "CSM", "INCUMPRIMENTO DE CONTRATOS"
            )
            ...
            documents, failed = await self.scrape_documents(
                links_to_scrape, self.dv_scraper, "CSM", "VIOLÊNCIA DOMÉSTICA"
            )
