...
from classification import BINARY_LABELS, LABEL_NAMES, TERNARY_LABELS, plot_forest

_REPO_ROOT = Path(__file__).resolve().parents[2]

results_df = pd.read_parquet(
    _REPO_ROOT / "data/results/evaluation_results_with_cis.parquet"
)
output_dir = _REPO_ROOT / "data/results/forest_plots"
...
for idx, row in results_df.iterrows():
    ...
    results = {
        "point_estimates": {
            ...
            "mcc": row["mcc_point"],
            # plot_forest only reads np.sum(...) of this to print "n=" in the
            # title; the consolidated parquet doesn't carry the real
            # per-class confusion matrix, but does carry the exact sample
            # count, so a 1x1 array with that count reproduces the same sum.
            "confusion_matrix": np.array([[row["n_samples"]]]),
        },
        "ci": {
            "macro_f1": {
