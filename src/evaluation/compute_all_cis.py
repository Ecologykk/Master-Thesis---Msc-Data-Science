...
# Model prediction file paths
PREDICTION_FILES = {
    "bert_v3_dv": "data/models/dl/dv/predictions/bert_v3_dv_predictions.parquet",
    "bert_v3_boc": "data/models/dl/boc/predictions/bert_v3_boc_predictions.parquet",
    "llm_zs_dv": "data/models/llms/deepseek_r1_8b/zero_shot_dv_predictions_smart9k.parquet",
    "llm_zs_boc": "data/models/llms/deepseek_r1_8b/zero_shot_boc_predictions_smart9k.parquet",
    "llm_fs_dv": "data/models/llms/deepseek_r1_8b/few_shot_dv_predictions.parquet",
    "llm_fs_boc": "data/models/llms/deepseek_r1_8b/few_shot_boc_predictions.parquet",
}

OUTPUT_PATH = Path("data/results/evaluation_results_with_cis.parquet")
...
def process_prediction_file(file_path, case_type, model_name, run_name):
    ...
    point_estimates = _evaluate_classification(y_true, y_pred, labels=labels, compute_tss=compute_tss)
...
def main():
    ...
    for key, rel_path in PREDICTION_FILES.items():
        ...
        if key.startswith("bert"):
            ...
            case_type = parts[2]  # "dv" or "boc"
            run_name = f"{'zero-shot' if strategy == 'zs' else 'few-shot'}_{case_type}_smart9k"
    ...
    print(results_df[[col for col in results_df.columns if "point" in col or "model_name" in col]].to_string())
